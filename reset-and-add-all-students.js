...
const studentManagementSchema = new mongoose.Schema({
    ...
    password: String,
    profilePhoto: String,
    dob: Date  // Changed from dateOfBirth to match server schema
});
...
async function resetAndAddStudents() {
    try {
        await mongoose.connect(process.env.MONGODB_URI);
        console.log('✅ Connected to MongoDB\n');
        
        // Delete all existing students
        const deleteResult = await StudentManagement.deleteMany({});
        ...
        for (const student of allStudents) {
            try {
                ...
                // Generate email: firstname.branchcode+enrollmentnumber@global.org.in
                // Example: aditya.cs231021@global.org.in
                const firstName = student.name.split(' ')[0].toLowerCase();
                const enrollmentLower = student.enrollment.toLowerCase(); // 0246cd241001
                const branchAndNumber = enrollmentLower.substring(4); // cd241001 (remove "0246")
                const email = `${firstName}.${branchAndNumber}@global.org.in`;
                
                // Create student
                const newStudent = new StudentManagement({
                    name: student.name,  // Only student's own name, not including father's name
                    fatherName: student.father,
                    enrollmentNo: student.enrollment,
                    course: 'B.Tech Data Science',
                    semester: '3',
                    dob: parsedDate,  // Changed from dateOfBirth to match server schema
                    password: 'aditya',
                    email: email,
                    phone: ''
                });
                ...
                console.error(`❌ Error: ${student.enrollment} - ${err.message}`);
                errors++;
            }
        }
